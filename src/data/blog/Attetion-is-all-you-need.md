...
- 卷积神经网络对长的序列难以建模，而attention可以一次看到整个序列。卷积的好是可以通过多种核做多个输出通道，不同输出通道可以认为识别了不同模式，所以这里就**用多头注意力机制来模拟卷积神经网络多输出通道的效果**。
...
- 计算`attention score`时将`padding` 的 `attention` 分数设为非常小的负数，使 `softmax` 后几乎为 0
```python
attention_scores = Q @ K.T / sqrt(dk)
attention_scores += (padding_mask * -1e9)
attention_weights = softmax(attention_scores)
```
- 做多头时每个头降维到 d_model/d_h,避免合并时维度爆炸和 `attention score` 过大,`softmax`后效果差
- 
...
- 输入词向量需要乘以根号下`embedding_dim`,使其与位置编码的量纲相匹配
...
- 是固定不可学的周期性编码，对模型来说不一定最优
## Training
- 一个工作是训练英语到德语，德语和英语共用一个37000 tokens的vocabulary
## Regularization
### LayerNorm
- `Transformer` 处理的是变长序列，`BatchNorm` 依赖的是整个 `batch`的均值和方差，而`LayerNorm`是针对每个样本自身的特征维度归一化，天然适用于语言模型等变长输入场景。
- `LayerNorm` 是在残差连接之后施加（也称为 `Post-Norm` 架构），后面有的研究表明 `Pre-Norm` 更稳定
...
- 如果把残差连接说明的去掉只有attention基本上什么都训练不出来，所以这个模型并不是只需要attention。残差结构保证了梯度的流动使模型可以一直训练下去。
