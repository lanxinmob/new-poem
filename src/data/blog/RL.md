...
*   **间接评估 (Indirect / Temporal Difference Learning, TD Learning):**
    ...
    *   从每一次单步经验中学习，不需要等游戏结束，只要走了一步 $(s, a, s', r)$，就利用贝尔曼方程的思想，把对未来状态 $s'$ 的估值“回传”给当前状态 $s$，通过**滑动平均**的方式增量更新 $V^\pi(s)$，用**采样**代替**期望**。但 TD Learning 只能评估固定的策略。正如之前讨论的，就算你算出了极其精确的 $V$ 值，由于你没有模型 $P$ 和 $R$，你依然**不知道该怎么改进策略（无法提取 $\pi^*$）**。
